...
module.exports = {
    ...
    addHuntToUser: (req, res, next) => {
        ...
        User.updateOne({ _id: mongoose.Types.ObjectId(user_id) }, { $addToSet: { hunts: [mongoose.Types.ObjectId(hunt_id)] } })
            .exec()
            .then((data, err) => {
            if (err) {
    ...
    deactivateHunt: (req, res, next) => {
        ...
        Hunt.updateOne({ _id: h_id }, { isActive: false })
            ...
            Team.updateMany({ hunt_id: h_id }, { lastClue_sent: 0, recall_Sent: false })
                .exec()
                .then((complete2, err2) => {
                if (err2) {
