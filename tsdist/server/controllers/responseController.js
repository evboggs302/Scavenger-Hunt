...
module.exports = {
    ...
    saveMMS: (req, res, next) => {
        ...
        const media = +req.body.NumMedia === 1
            ? req.body.MediaUrl0
    ...
    markResCorrect: (req, res, next) => {
        ...
        Response.findOne({ _id: res_id }).then((response, err) => {
            ...
            Response.updateOne({ _id: res_id }, { correct: true }).then((data, err) => {
                if (err) {
    ...
    sendClue: (req, res, next) => {
        let { team_id, nextClue, recall_sent, device_number, recallMessage } = req.body;
        const t_id = mongoose.Types.ObjectId(team_id);
        if (nextClue) {
            Team.updateOne({ _id: t_id }, { lastClue_sent: nextClue.order_number }).then((complete, err) => {
                if (err) {
            ...
            Team.updateOne({ _id: t_id }, { recall_sent: true }).then((complete, err) => {
                if (err) {
    ...
    deleteAllResponsesByHunt: (req, res, next) => {
        ...
        Team.aggregate([
            ...
            if (err) {
                ...
                Response.bulkWrite(bulkTeamIds, { ordered: false }).then((data, err) => {
                    if (err) {
