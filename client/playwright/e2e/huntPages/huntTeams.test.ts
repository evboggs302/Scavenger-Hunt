...
e2eTest("Hunt Teams page", async ({ page, TeamsPage }) => {
  const {
    teamsTable,
    createDialog,
    deleteAllDialog,
  ...
  await e2eTest.step("CREATE - 1 team", async () => {
    const teamMembersInput = createDialog.getByTestId("create-team-members-0");
  ...
  await e2eTest.step("UPDATE - content", async () => {
    const teamMembersInput = updateDialog.getByTestId("create-team-members-0");
